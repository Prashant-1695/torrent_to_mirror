...
UPLOAD_COUNT=0
FAILED_COUNT=0
LOG_FILE="/tmp/upload_log_$(date +%Y%m%d_%H%M%S).log"
...
load_cirrus_config() {
    ...
    if [[ -f .cirrus.yml ]]; then
        log_info "Loading configuration from .cirrus.yml..."
        
        # Extract variables with better parsing
        PIXELDRAIN_API_KEY=$(yq eval '.env.PIXELDRAIN_API_KEY // ""' .cirrus.yml 2>/dev/null || \
                              grep -A 20 "env:" .cirrus.yml | grep "PIXELDRAIN_API_KEY:" | \
                              sed 's/.*PIXELDRAIN_API_KEY: *//;s/ENCRYPTED\[//;s/\]$//' | tr -d '"' || echo "")
        
        BOT_ID=$(yq eval '.env.BOT_ID // ""' .cirrus.yml 2>/dev/null || \
                 grep -A 20 "env:" .cirrus.yml | grep "BOT_ID:" | \
                 sed 's/.*BOT_ID: *//;s/ENCRYPTED\[//;s/\]$//' | tr -d '"' || echo "")
        
        CHAT_ID=$(yq eval '.env.CHAT_ID // ""' .cirrus.yml 2>/dev/null || \
                  grep -A 20 "env:" .cirrus.yml | grep "CHAT_ID:" | \
        ...
        log_warn ".cirrus.yml not found, using .env as fallback..."
        set -a  # automatically export all variables
        source .env
        set +a
    else
        log_error "No configuration file found (.cirrus.yml or .env)"
        return 1
    fi
}
...
📂 Working Directory: \`$(pwd)\`"
    ...
    # Add current directory if no Downloads folder or if explicitly requested
